...
export default function BlogPage() {
  ...
  return (
    <>
      <section className="pt-24 sm:pt-32 pb-12 relative overflow-hidden">
        ...
        <div className="relative container-x">
          ...
          <h1 className="section-title-lg mt-6 text-balance">
            Reading <span className="italic text-oxblood-600">for the collector.</span>
          </h1>
            ...
            >
              <div className="grid lg:grid-cols-2 gap-8 lg:gap-14">
                ...
                <div className="flex flex-col justify-center">
                  <div className="text-[10px] uppercase tracking-[0.4em] text-oxblood-600 mb-4">
                    Featured · {hero.category} · {hero.readTime}
                  </div>
                  <h2 className="font-serif text-3xl sm:text-4xl lg:text-5xl leading-tight group-hover:text-oxblood-600 transition-colors text-balance">
                    {hero.title}
                  </h2>
                  <div className="hairline-gold mt-6 w-16" />
                  <p className="mt-6 text-ink-600 leading-relaxed text-lg font-light">{hero.excerpt}</p>
                  <div className="mt-8 flex items-center gap-4 text-xs text-ink-400 italic font-serif">
                    <span>{hero.author}</span>
                    <span className="text-oxblood-600/50">·</span>
                    <time>{new Date(hero.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</time>
                  </div>
                  <div className="mt-8 text-[10px] uppercase tracking-[0.4em] text-oxblood-600">Read Article →</div>
                </div>
              </div>
            </Link>
          </div>
      ...
      <section className="pb-24 sm:pb-32 border-t border-ink-100 pt-16">
        <div className="container-x">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 lg:gap-14">
            {rest.map((p) => (
              <Link
                ...
                className="group block"
              >
                <div className="aspect-[4/5] overflow-hidden bg-bone-50 mb-6">
                ...
                <h3 className="font-serif text-2xl leading-tight group-hover:text-oxblood-600 transition-colors text-balance">{p.title}</h3>
                <p className="text-sm text-ink-500 mt-4 leading-relaxed line-clamp-2">{p.excerpt}</p>
