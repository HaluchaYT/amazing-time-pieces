...
export default function BlogPostPage({ params }) {
  ...
  return (
    <>
      <article>
        <header className="pt-12 sm:pt-16 pb-10">
          <div className="container-narrow max-w-3xl">
            ...
            <div className="mt-8 flex items-center gap-4 text-sm text-ink-500 italic font-serif">
        ...
        <div className="container-x max-w-5xl">
          <div className="editorial-frame aspect-[16/9] overflow-hidden bg-bone-50">
            <img src={post.cover} alt={post.title} className="w-full h-full object-cover" />
          </div>
        </div>

        <div className="container-narrow max-w-3xl py-16 sm:py-20">
          <div className="prose-content space-y-8 text-lg text-ink-700 leading-[1.9] font-light">
            {post.content.map((para, i) => (
              <p key={i} className={i === 0 ? 'first-letter:font-serif first-letter:text-6xl first-letter:font-normal first-letter:float-left first-letter:mr-3 first-letter:mt-1 first-letter:text-oxblood-600' : ''}>
                {para}
              </p>
            ))}
          </div>
          ...
          <div className="mt-16 pt-8 border-t border-ink-100">
            ...
            <div className="flex flex-wrap gap-2">
              {post.tags.map((t) => (
                <span key={t} className="text-[10px] uppercase tracking-[0.35em] border border-oxblood-600/30 text-ink-600 px-3 py-1.5">
                  {t}
                </span>
              ))}
            </div>
      ...
      <section className="py-20 sm:py-28 border-t border-oxblood-600/15 bg-bone-100">
        <div className="container-x">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            {related.map((p) => (
              ...
              >
                <div className="aspect-[4/5] overflow-hidden bg-bone-50 mb-5">
                ...
                <h3 className="font-serif text-xl leading-tight group-hover:text-oxblood-600 transition-colors text-balance">{p.title}</h3>
              </Link>
            ))}
          </div>
        </div>
