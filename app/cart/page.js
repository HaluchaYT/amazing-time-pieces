      ...
      <section className="py-12 sm:py-16">
        <div className="container-x">
          <div className="flex items-baseline justify-between mb-8">
            ...
            <Link href="/watches" className="text-xs uppercase tracking-widest text-gold-400 link-underline hidden sm:inline">
              ← Continue Shopping
            </Link>
          </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                {items.map((item) => (
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col sm:flex-row justify-between gap-4">
                      <div>
                        <div className="text-[10px] uppercase tracking-widest text-gold-400/80">{item.brand}</div>
                        <Link href={`/watches/${item.id}`} className="block font-serif text-xl sm:text-2xl mt-1 hover:text-gold-300 transition-colors">
                          {item.model}
                        </Link>
                        <div className="text-xs text-white/50 mt-1">Ref. {item.reference}</div>
                        <button
                          onClick={() => remove(item.id)}
                          className="mt-4 text-xs uppercase tracking-widest text-white/50 hover:text-gold-400 transition-colors"
                        >
                          Remove
                        </button>
                      </div>
                      <div className="text-left sm:text-right">
                        <div className="font-serif text-2xl text-gold-400">{formatPrice(item.price)}</div>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <aside className="lg:sticky lg:top-28 self-start bg-ink-900 border border-white/10 p-6 sm:p-8">
                ...
                <div className="mt-6 pt-6 border-t border-white/10 flex justify-between items-baseline">
                  ...
                  <span className="font-serif text-3xl text-gold-400">{formatPrice(subtotal)}</span>
                </div>
                ...
                <div className="mt-6 space-y-3 text-xs text-white/50">
                  <div className="flex items-center gap-2">
                    <span className="text-gold-400">✓</span> Pay by cash, wire, or Zelle
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-gold-400">✓</span> 7-day inspection window
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-gold-400">✓</span> Fully insured worldwide delivery
                  </div>
                </div>
              </aside>
            </div>
